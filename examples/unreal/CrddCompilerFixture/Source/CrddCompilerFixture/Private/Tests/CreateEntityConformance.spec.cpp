...
#include "Generated/CreateEntity.generated.h"
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCrddCreateEntityConformanceTest,
    "CRDD.CreateEntity.Conformance",
...
)
{
    const FString BundlePath = FPaths::ConvertRelativePathToFull(
        FPaths::Combine(
            FPaths::ProjectDir(),
            TEXT("../../../generated/create-entity.conformance.json")
        )
    );
...
FCrddCreateEntityState ReadState(const TSharedPtr<FJsonObject>& StateObject)
{
    ...
    const TArray<TSharedPtr<FJsonValue>>& Entities = StateObject->GetArrayField(TEXT("entities"));
    for (const TSharedPtr<FJsonValue>& EntityValue : Entities)
    {
        const TSharedPtr<FJsonObject> Entity = EntityValue->AsObject();
        State.Entities.Add(
            {
                Entity->GetNumberField(TEXT("length")),
                static_cast<int64>(Entity->GetNumberField(TEXT("cost")))
            }
        );
    }
    return State;
...
void CompareState(
    ...
    const FCrddCreateEntityState& Actual,
    const FCrddCreateEntityState& Expected
)
{
    Test.TestEqual(
    ...
    Test.TestEqual(
        FString::Printf(TEXT("%s entity count"), *CaseId),
        Actual.Entities.Num(),
        Expected.Entities.Num()
    );

    const int32 Count = FMath::Min(Actual.Entities.Num(), Expected.Entities.Num());
    for (int32 Index = 0; Index < Count; ++Index)
    {
        Test.TestEqual(
            FString::Printf(TEXT("%s entity[%d] length"), *CaseId, Index),
            Actual.Entities[Index].LengthMeters,
            Expected.Entities[Index].LengthMeters
        );
        Test.TestEqual(
            FString::Printf(TEXT("%s entity[%d] cost"), *CaseId, Index),
            Actual.Entities[Index].CostJPY,
            Expected.Entities[Index].CostJPY
        );
    }
}
}

bool FCrddCreateEntityConformanceTest::RunTest(const FString& Parameters)
{
    ...
    for (const TSharedPtr<FJsonValue>& CaseValue : Cases)
    {
        ...
        FCrddCreateEntityInput OperationInput;
        ...
        const FCrddCreateEntityResult Actual = FCrddCreateEntityOperation::Execute(
            OperationInput,
        ...
        TestEqual(
            ...
            FCrddCreateEntityOperation::ErrorCode(Actual.Error),
            ExpectedError
        );
